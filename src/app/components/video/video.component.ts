          ...
          if (poseState.pose) {
            ...
            if (settingsState.drawPose) {
              try {
                ...
                // Update PoseService.draw to accept and honor this option for best effect.
                // eslint-disable-next-line @typescript-eslint/ban-ts-comment
                // @ts-ignore
                this.poseService.draw(poseState.pose, ctx, {drawFace: false});
              } catch (e) {
                // If the PoseService API doesn't accept options, fall back to the original call
                // so nothing breaks. To permanently remove face drawing, edit PoseService.draw
                // or the drawing utilities to ignore faceLandmarks.
                // (Log a warning to help you identify the change needed.)
                // eslint-disable-next-line no-console
                console.warn('poseService.draw(options) not supported — falling back. To remove face/ avatar drawing, update PoseService.draw to accept {drawFace: false}.', e);
