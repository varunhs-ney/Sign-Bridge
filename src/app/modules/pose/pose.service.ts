...
export class PoseService {
  ...
  private async _load(): Promise<void> {
    ...
    await this.ga.trace('pose', 'load', async () => {
      ...
      // Remove any image/segmentationMask from results before forwarding to callbacks.
      // This prevents downstream code from drawing the original video frame or segmentation mask.
      this.model.onResults(results => {
        // Create a shallow copy and delete image & segmentationMask if present
        const safeResults: any = Object.assign({}, results);
        try {
          // Some versions use segmentationMask, some may use other names — remove common ones
          delete safeResults.image;
          delete safeResults.segmentationMask;
          delete safeResults.segmentationMaskData;
        } catch (e) {
          // ignore
        }

        for (const callback of this.onResultsCallbacks) {
          // Forward only the safeResults (no image/mask)
  ...
  // kept for reference but NOT used; face drawing has been disabled deliberately
  ...
  /**
   * Draw the pose and hands. Face drawing has been intentionally disabled so only skeleton and hands are rendered.
   * This function now clears the canvas first to avoid showing any leftover image beneath the skeleton.
   */
  draw(pose: EstimatedPose, ctx: CanvasRenderingContext2D, _opts?: { drawFace?: boolean }): void {
    // clear canvas to ensure no underlying image/segmentation is visible
    ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height);

    // save/restore context to be safe (older code called ctx.restore() at the end)
    ctx.save();
    ...
    // deliberate: DO NOT draw face/avatar at all
